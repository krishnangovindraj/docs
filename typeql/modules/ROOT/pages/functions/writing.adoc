...
- A *return statement* that produces results matching the specified return.

.Function Declaration Syntax
[,typeql]
----
fun <function-name> ( <optional-arguments> ) -> <return-specification>:
<read-pipeline>
...
- `<return-specification>` consists of at least one `<return-type>` and can be:
* a xref:{page-version}@typeql::functions/stream.adoc[stream] of tuples, written in the format `{ A, B, ... }`.
* a (single) xref:{page-version}@typeql::functions/tuple.adoc[tuple], written in the format `A, B, ...`.
...
// Do we want to support scalar functions?
...
`$a;`.
// Don't These always have to be reductions? (or first, last, check)
